...
const handleLogin = (e) => {
    e.preventDefault();
    signInWithEmailAndPassword(auth, email, password).then((getdata) => {

        navigate('/home');
    })
        .catch(() => { seterr("Error signing please try again") })
...
return (
    <div className="flex justify-center items-center h-screen bg-gray-100">
        <form onSubmit={handleLogin} className="p-10 bg-white rounded-lg shadow-md" style={{ width: "75%" }}>
            ...
            <p className='text-red-500 cursor-pointer my-2' onClick={() => navigate("/signup")}>{err}</p>
